...
Among the most studied ribozymes, the hairpin ribozyme (HpR) stands out for its ability to cleave a phosphodiester bond within its own backbone, replacing a 3'-5' polymeric linkage with a 2'-3' cyclic terminating linkage \cite{fedor_structure_2000,muller_many_2012}.
Although HpR is not the only self-reactive ribozyme, it is one of the few capable of catalyzing this reaction without the requirement for divalent metal ions \cite{chowrira_extensive_1992, hampel_unique_1997, nesbitt_unusual_1997, young_metal_1997, nesbitt_internal_1999}.
...
Mutagenesis experiments \cite{kuzmin_role_2005}, reinforced by structural resolution studies \cite{rupert_crystal_2001,salter_water_2006,spitale_identification_2009,macelrevey_structural_2008}, have demonstrated that two surrounding nucleobases, G8 and A38, are mandatory for the chemical activity and are in the direct neighbourhood of the A-1 --- G+1 junction.
From there, a first mechanistic scenario was proposed \cite{bevilacqua_mechanistic_2003}: the dianionic scenario proposes that both G8 and A38 are directly involved, acting respectively as the general base that activates the nucleophile O2' by capturing its proton, and as the general acid that reprotonates the leaving group O5' after the elimination step (see Figure \ref{fig:all_mecas} A) \cite{bevilacqua_mechanistic_2003}.
Despite supporting pH-dependency experiments \cite{wilson_nucleobase_2006, kath-schorr_general_2012} and matching transition state arrangements \cite{rupert_transition_2002}, this cleavage scenario suffers from one major inconsistency, which is the necessary prior deprotonation but \textit{in vivo} impracticality of the N1 atom of the G8 nucleobase \cite{cottrell_ph_2011}.
...
network which are coherent wiht the general dianionic scenario: G8:N1···A-1:O2’, A38:N1···G+1:O5’, G8:N2···G+1:Opro-Rp, and A38:N6···G+1:Opro-Rp \cite{mlynsky_reactive_2015,veenis_investigation_2021}.
Although a certain stability of this local conformation was evaluated within 500~ns of simulation,
we proved i our previous work that the stability of this local arrangement, which we labeled L$_\mathrm{1}$, 
...
L$_\mathrm{2}$, is the true stable conformation of the hairpin ribozyme in its pre-calaytic state.
These observations are in line with the second pitch which stipulates that this L$_\mathrm{2}$ arrangement,
in which G8 is protonated and distinct from O2' and O2' involved in a H-bond with the non-brdiging pro-Rp oxygen, could be the departure point of the in-line monoanionic scenario. 
...
In this study, we propose to complement the arsenal of \textit{in sillico} studies which have conducted on the HpR and to explore the conformational landscape of the different intermediates of the two opposed scenarios within a comparative study,
and using state of the art extensive sampling techniques, Replica Exchange with Solute Tempering or REST2, which have proven to be essential in the study of systems as flexible as the HpR \cite{forget2024simulation}. 
This will lead us to yet so far undone simulation of the pro-Sp monoprotic HpR, and several cleaved states of the HpR.
With this, we provide valuable insights into the mechanistic debate, by addressing the feasibility of the mechanistic steps at each stage of the reaction.
...
The parameters for the monoprotic phosphate group with the proton on the pro-Sp atom (labeled RSA + GSA) were adapted from these very RAA and GAA parameters by simply inverting the charge of pro-Rp and pro-Sp oxygens, without modifying the rest of the redistribution.
...
We chose to initiate the simulations of the ligated dianionic scenario intermediates from the crystallographic structure of the pre-catalytic state of the minimal HpR (PDB ID: 2OUE) \cite{rupert_crystal_2001},
...
The pro-Rp monoprotic intermediate HpR simulation was initiated from a L2 conformation 
...
we used a L$_\mathrm{1}^{pucker}$ frame from the trajectory provided by Mlynsky et al. (see our previous work for conformational labelling \cite{forget2024simulation} for more details on this simulation).
...
The positions of the final catalytic site atoms (in the crystal structure) were compared to the only other cleaved HpR which has been resolved, which differs in sequence and size while conserving the same satalytic core (PDB ID: 1M5V)
We demonstrate excellent consistency, with an RMSD of 0.93~\AA\ calculated over all heavy atoms of residues A-1, G+1, G8 and A38 (see Figure \ref{fig:cleaved_crystals}).
...
Shortly, the solvent was progressively equilibrated with a 5000 step energy minization, a 500~ps NVT and a NPT equilibration at $\mathrm{T}= 298.15$~K 
...
All bonds with hydrogen atoms are subjected to the LINCS algorithm \cite{hess_lincs_1997}, and the simulation timestep was always set at 2~ps. 
The production might then start from there. Gromacs 2022.3 \cite{abraham_gromacs_2015} patched with PLUMED 2.7 \cite{tribello_plumed_2014, the_plumed_consortium_promoting_2019} was used for all simulations and some system preparation. 
...
The simuations typically lasted for 500~ns, and displayed a average exchange rate of 15---20$\%$.  
...
This labelling system is entirely based on the combination of observables implying the catalytic site atoms, and distiguish three main conformations.
...
The convergence of cleaved states was assessed using only the eRMSD global metric only, as  we did not settle any satisfying local collective variable (see Figure\ref{fig:Int_REST2_Convergence} E-F).
...
Deprived of this knowledge, we choose here to consider accordingly every species that could appear along the different mechanistic pathways,s
starting with description of certain of the in-line mono-anionic scenario states and ending with those of the general dianionic scenario.
...
%Along this mechanism, several protonation states of the catalytic site emerge. We will abusively call them intermediate although we have no evidence that these species correspond to local minima and show any metastablity.

%% ISSINF Here figure of O1P- O2' vs O2P-2' in pre-cat
